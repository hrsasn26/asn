...
**Placeholders.** Les informations non confirmées restent entre crochets : `[Ville ou région]`, `[pays à définir]`, `[À rédiger : …]`. Elles sont dans le brief ou dans `src/data/`, pas sur les pages. Le script `pnpm check:content` liste les placeholders affichés. En mode `--strict`, il bloque la mise en production tant qu'il en reste.
