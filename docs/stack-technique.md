...
| SEO et GEO | `@astrojs/sitemap`, JSON-LD, `llms.txt` | Balises title et meta du brief. Données structurées : ProfessionalService (avec l'adresse et le téléphone de l'agence, identifiant `@id` repris par Service et WebSite), WebSite, Service, BreadcrumbList, FAQPage (produit par le composant `Faq` à partir des questions affichées), sans zone servie tant qu'elle n'est pas confirmée. Fichier `/llms.txt` (format llmstxt.org) pour les assistants et les moteurs à base d'IA : les coordonnées de l'agence, puis une ligne par page, avec sa meta description. |
...
- **Placeholders cachés** : depuis le 26 septembre 2026, le site n'affiche aucun placeholder, pages légales et coordonnées comprises. Une phrase écrite dans une page et qui attend une information est retirée : elle reste dans le brief avec son placeholder (pages légales : section 7). Une valeur de `src/data/` encore entre crochets (conditions des CGV) n'est pas affichée : la page teste la valeur avec `estConfirme` (`src/lib/placeholders.ts`), et la phrase apparaît quand la vraie valeur remplace le placeholder.
- **Placeholders surlignés** : un placeholder affiché par erreur apparaît sur fond jaune (classe `a-completer`), pour la relecture. Le composant `Texte` s'en charge pour une phrase, `Prose` pour les pages légales.
...
- Logo (archive du 26 septembre 2026, `design/logo/`) : un symbole de deux demi-disques décalés et le nom sur deux lignes (composant `Logo`). Version couleur dans l'en-tête, version blanche dans le pied de page. Animé en CSS (section « Animations » ci-dessous). Le même symbole sert pour le favicon, l'icône Apple, les données structurées et l'image de partage.
...
- logo (composant `Logo`) : au survol et au focus clavier du lien de l'en-tête, les deux demi-disques du symbole se rejoignent en un disque plein (`logo-demi`) ; à l'ouverture de l'accueil seulement, ils arrivent des côtés, se rejoignent, puis se séparent avec un rebond (`anim-logo`, 1,2 seconde). Sur les autres pages, l'en-tête reste immobile pendant la transition de page. Le nom n'est pas animé ;
...
**Placeholders.** Les informations non confirmées restent entre crochets : `[numéro de récépissé]`, `[pays à définir]`, `[À rédiger : …]`. Elles sont dans le brief ou dans `src/data/`, pas sur les pages. Le script `pnpm check:content` liste les placeholders affichés. En mode `--strict`, il bloque la mise en production tant qu'il en reste.
