...
| SEO et GEO | `@astrojs/sitemap`, JSON-LD, `llms.txt` | Balises title et meta du brief. Données structurées : ProfessionalService (avec l'adresse et le téléphone de l'agence), Service, BreadcrumbList, sans zone servie tant qu'elle n'est pas confirmée. Fichier `/llms.txt` (format llmstxt.org) pour les assistants et les moteurs à base d'IA : les coordonnées de l'agence, puis une ligne par page, avec sa meta description. |
