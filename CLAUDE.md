...
- Pages légales : projets de texte, à faire valider par un juriste marocain. Depuis le 26 septembre 2026, elles n'affichent que les parties sans placeholder. Les parties retirées sont dans la section 7 du brief (« Parties retirées des pages légales »). Le téléphone est aussi caché (Contact, mentions légales) jusqu'à ce que le numéro soit disponible.
...
- Ne jamais inventer de prix, de statistiques, de témoignages ou de références clients. Une phrase qui dépend d'une valeur non confirmée (délai, pays, ville, adresse, zone servie, durée, identifiants de l'entreprise, clause à rédiger) n'est pas publiée, ni sur les pages, ni dans les données structurées, ni dans `llms.txt` : elle reste dans le brief avec son placeholder. Cela vaut aussi pour les pages légales et les coordonnées. Une valeur de `src/data/` encore entre crochets (`[numéro de téléphone]`, conditions des CGV) n'est pas affichée : testez-la avec `estConfirme` (`src/lib/placeholders.ts`).
