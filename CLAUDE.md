...
- FAQ : les questions sont un tableau `QuestionFaq[]` dans la page (`src/lib/faq.ts`), passé à `<Faq questions={…} />`. Le composant affiche la FAQ et produit les données structurées FAQPage à partir des mêmes textes. `tests/e2e/donnees-structurees.spec.ts` vérifie qu'ils sont identiques.
